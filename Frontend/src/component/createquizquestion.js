...
const Createquizquestion = ({ onQuestionsChange }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("https://quiz-setx.onrender.com/quizsetup/questionForonequiz");
  ...
  const handleDelete = async (question_id) => {
    try {
      await axios.post("https://quiz-setx.onrender.com/quiz/deletequestion", { question_id });
  ...
  const setQuizForExam = async () => {
    try {
      ...
      const response=await axios.post("https://quiz-setx.onrender.com/quiz/setQuizNameToFile", { name });
